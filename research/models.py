...
"""
def Nest_Net(img_rows=args.height, img_cols=args.width, color_type=1, num_class=1, deep_supervision=False):
    nb_filter = [32, 64, 128, 256, 512]
    global bn_axis
    bn_axis = 3
    ...
    if deep_supervision:
        model = tf.keras.Model(input=img_input, output=[nestnet_output_1,
                                                     nestnet_output_2,
                                                     nestnet_output_3,
                                                     nestnet_output_4])
    else:
